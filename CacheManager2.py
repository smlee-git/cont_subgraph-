...
class Trie() :
    ...
    def searchAndReturn(self, node, id) :
        for c in node.children :
            if c.id == id :
                return c
            self.searchAndReturn(c, id)
        return None
...
def deleteEdges(pCache, uCache, s, dId, flag) :
    # dId = s[0].id
    dv1 = s[1][0]
    dv2 = s[1][1]
    item = getDataFromCache(pCache, uCache, dId)
    if item is not None :
        if flag == 0 :
            idx = 0
            for de in item['e'] :
                if de[0] == dv1 and de[1] == dv2 :
                    del item['e'][idx]
                    item["edge_size"] -= 8
                    if dId in pCache :
                        pCache["total_size"] -= 8
        ...
        fflag = False
        for idx in range(len(item['r'])-1, -1, -1) :
            dr = item['r'][idx]
            if dv1 in dr and dv2 in dr :
                del item['r'][idx]
                item["result_size"] -= len(dr)*4
                if dId in pCache :
                    pCache["total_size"] -= len(dr)*4
                else :
                    uCache["total_size"] -= len(dr)*4
                fflag = True

        if fflag and len(s[0].children) > 0 :
            for c in s[0].children :
                deleteEdges(pCache, uCache, s, c.id, 1)
    else :
        lines = []
        if flag == 0 :
            with open(f"nodes/{dId}_edge", "r") as file:
                lines = file.readlines()
                for line in lines : # 부모의 정보를 읽어와서 한줄씩 처리
                    line2 = list(map(int, line.strip().split(" "))) #엔터가 들어가서 ..
                    if line2[0] == dv1 and line2[1] == dv2 :
                        lines.remove(line)
                        break

            with open(f"nodes/{dId}_edge", "w") as file2:
                # print('------------------------------------------', data)
                file2.seek(0)
                for line in lines: #edge 저장
                    file2.writelines(line)
                file2.flush()

        lines = []
        fflag = False
        with open(f"nodes/{dId}_result", "r") as file3:
            lines = file3.readlines()
            for line in lines : # 부모의 정보를 읽어와서 한줄씩 처리
                line2 = list(map(int, line.strip().split(" "))) #엔터가 들어가서 ..
                if dv1 in line2 and dv2 in line2 :
                    lines.remove(line)
                    fflag = True
        if fflag :
            with open(f"nodes/{dId}_result", "w") as file4:
                # print('------------------------------------------', data)
                file4.seek(0)
                for line in lines: #edge 저장
            ...
            for k in tries :
                rtv = tries[k].searchAndReturn(tries[k].node, dId)
                if rtv is not None and len(rtv.children) > 0 :
                    for c in rtv.children :
                        deleteEdges(pCache, uCache, s, c.id, 1)
...
with open(filename, "r") as f:
    ...
    percentage = 0.001 # 1perent = 10M, 0.1 1M
    ...
    while 1 :
        ...
        if (t > 1 and last_time < t and last_time % 1 == 0):

            # print("Time Stamp Changed !!", t)
            end = time.time()
            # if len(streamList) > 0 :
            if t > 300 :
                # drange = int(len(streamList)*deleteRatio)
                for sidx in range(0, drange, 1) :
                    deleteEdges(pCache, uCache, streamList[sidx], streamList[sidx][0].id, 0)
                # int(100*deleteRatio) print()
                # print(drange)
                streamList = streamList[drange :]
        ...
        for currentNode in currentNodes :
            ...
            # cres['res_size'] += 8
            cres['e'].append(data)
            streamList.append([currentNode, data])
